...
<div class="contenedor">
    ...
    <div class="acciones-rapidas" style="margin-top: 40px;">
    ...
    $res_pedidos = mysqli_query($conn, "SELECT descripcion_cliente FROM pedidos");

    $correos = [];

    while ($pedido = mysqli_fetch_assoc($res_pedidos)) {
        $texto = $pedido['descripcion_cliente'];
        
        if (preg_match('/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}\b/i', $texto, $matches)) {
            $correos[] = $matches[0];
        }
    }

    $correos_unicos = array_unique($correos);
    $total_clientes = count($correos_unicos);
